...
Quadcopter.prototype._stabilize = function (done) {
	...
	this._readOrientation(function (err, data) {
		...
		// Quadcopter turned off or on ground, do not stabilize
		if (!that.enabled) {
			return done(null);
		}
...
Quadcopter.prototype._resetMotorSpeeds = function () {
    var that = this;
    var minSpeed = that.config.servos.range[0];
    this.config.servos.pins.forEach(function (pin, i) {
		that.motors.write({ pin: pin, value: minSpeed });
	});

	this._props.motorsSpeed = [0, 0, 0, 0];
};
