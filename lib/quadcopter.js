...
Quadcopter.prototype.start = function (done) {
	...
	loadSensors(this, function (err, sensors) {
		...
		that.server.listen(function () {
			...
			that.camera.isAvailable(function (cameraAvailable) {
...
Quadcopter.prototype._stabilize = function (done) {
	...
	this._readOrientation(function (err, data) {
		...
		if (that.pidEnabled/* && that.power >= 0.2*/) {
			...
			// grams to kg
			var structM = physics.structureMass / 1000;
			var motorM = physics.motorMass / 1000;

			var d = physics.diagonalLength / 100; // cm to m, distance between two motors
			
			var Q = 1 / (d / 2 * (structM / 3 + 2 * motorM));

			delta.force = {};
			for (var axis in delta.rate) {
				var d = delta.rate[axis];

				/*// Convert angular speed to angular acceleration
				var da = d / dt;

				// Convert angular acceleration to force
				var df = da * m * l / 3;*/
